...
- 3장 스프링 웹 기술과 스프링 MVC
      ...
      - MVC 아키텍처는 프론트 컨트롤러 패턴과 함께사용
        ...
        - DispatcherServlet의 DI 가능한 전략
          ...
          - HandlerAdapter
            ...
            - 핸들러 매핑과 어댑터가 한가지 컨트롤러에만 적용되기도함(DefaultAnnotationHandlerMapping(spring 3.2 이상부터 RequestMappingHandlerMapping) 에 의해 핸들러 결정되고, AnnotationMethodHandlerAdapter(spring 3.2 이상부터 RequestMappingHandlerAdapter)에 의해 핸들러 호출)
          ...
          - RequestToViewNameTranslator
            - 컨트롤러에서 뷰 이름이나 뷰 오브젝트를 제공해주지 않았을 경우 URL과 같은 요청정보를 참고해서 자동으로 뷰이름을 생성해주는 전략
  ...
  - 컨트롤러(Controller)
    ...
    - 컨트롤러의 종류와 핸들러 어뎁터(DispatcherServlet이 HandlerMapping을 통해서 세부 컨트롤러를 찾으면, 해당 세부컨트롤러를 실행시켜줄 HandlerAdapter를 찾는다)
      ...
      - AnnotationMethodHandlerAdapter(스프링 3.2 이상 RequestMappingHandlerAdapter)
        ...
        - DefaultAnnotationHandlerMapping(스프링3.2 이상 RequestMappingHandlerMapping) 핸들러 매핑과 함께 사용됨
        - 규칙과 관례를 잘 기억해야함!
    - 핸들러 매핑
      ...
      - 종류
        ...
        - DefaultAnnotationHandlerMapping(디폴트, 스프링 3.2 이상 RequestMappingHandlerMapping 변경)
