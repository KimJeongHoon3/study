...
- 4장 스프링 @MVC
    ...
    - @RequestMapping 핸들러 어뎁터 : RequestMappingHandlerAdpater
      ...
      - @Valid와 @RequestBody
        ...
        - ~~BindingResult 또한 사용가능~~
          - 사용불가!! @RequestBody에서 @Valid로 검증하면 예외처리는 인터셉터(ControllerAdvice)나 @ExceptionHandler 로 할것!
          - BindingResult로 넘어가지않기때문에 그냥 에러 뱉는다.. 그걸 잡아서 처리해야하는 개념으로 접근해야하기에 인터셉터나 ExceptionHandler 사용필요..
  ...
  - 4장을 통한 기타 팁
    ...
    - <span style="color:yellow">MockMvc는 에러발생시, 인터셉터시 postHandle과 afterCompletion 의 로그는 찍지않던데, 예외핸들러를 실행시킨뒤 afterCompletion은 왜 안탈까?</span>
      - HandlerInterceptor로 확인해본바, preHandle의 로그는 남으나, postHandle과 afterCompletion의 로그는 안남음
