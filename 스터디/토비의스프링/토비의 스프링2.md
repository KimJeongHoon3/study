...
- 4장 스프링 @MVC
      ...
      - 바인딩 과정에서 변환 작업을 위한 API
        ...
        - Validator와 BindingResult, Error
          ...
          - BindingResult와 MessageCodeResolver
            - BindingResult에는 모델의 바인딩 작업중에 나타난 타입 변환 오류정보와 검증 작업에서 발견된 검증 오류정보가 모두 저장
  ...
  - 메세지 컨버터와 AJAX
    ...
    - 메시지 컨버터의 종류
      ...
      - StringHttpMessageConverter
        ...
        - 미디어 타입은 모든것을 지원
        - @ResponseBody로 보낼때는 콘텐트 타입이 test/plain으로 지정
    ...
    - 새로운 RequestMapping 전략
      ...
      - HandlerMethod
        ...
        - 컨트롤러 오브젝트 대신 추상화된 메소드 정보를 담은 HandlerMethod를 결과로 돌려주고, 핸들러 어뎁터는 HandlerMethod 오브젝트의 정보를 이용해 메소드를 실행
          - 여기서 중요한것은 HandlerMethod는 그 자체가 실행 가능한 오브젝트가 아니다!! 대신, 메소드를 실행하는데 필요한 참조정보를 담고있는 오브젝트! 핸들러어뎁터를 이 참조정보를 활용해서 간접적으로 @RequestMapping의 메소드를 실행
        - HandlerMethod에 담긴정보
          - 빈 오브젝트
          - 메소드 메타정보
          - 메소드 파라미터 정보
    ...
    - @RequestMapping 핸들러 어뎁터 : RequestMappingHandlerAdpater
      ...
      - 3.1의 확장포인트 (RequestMappingHandlerAdpater로 변경되었으니 확장하는 클래스도 변경!)
        ...
        - HandlerMethodReturnValueHandler
          - 리턴값 처리하는 기능 확장
          - 3.1에 맞추어, ModleAndViewResolver 를 대체
  ...
  - 4장을 통한 기타 팁
    ...
    - Post로 전송할때 parameter로 보내는것과 body 보내는거는 다른거다......
      - parmater를 오브젝트로 변경해주는게 @ModelAttribute
      - body를 오브젝트로 변경해주는게 @RequestBody
...
- 6장 테스트 컨텍스트 프레임워크
  ...
  - 테스트용 컨텍스트의 공유는 메소드내 뿐 아니라, 클래스 사이에서도 가능!
    - 엄밀하게 이야기하면 테스크 컨텍스트 프레임워크가 애플리케이션 컨텍스트를 관리해서 같은 컨텍스트 사용하면 새로 만드는것이 아니라 계속 사용할수 있도록 해주는것임!
...
- 7장 스프링의 기타 기술과 효과적인 학습방법
  - 빈을 효과적으로 분석하는 방법
    ...
    - 위 두가지 특징을 잘 알고 싶다면, 빈으로 등록된 클래스의 구현 인터페이스와 프로퍼티 목록을 살펴보면됨
  ...
  - IoC 컨테이너 DI
    ...
    - 가장많이 사용되는 IoC 컨테이너의 확장포인트는 BeanPostProcessor와 BeanFactoryPostProcessor
      ...
      - BeanFactoryPostProcessor(빈 팩토리 후처리기)
        ...
        - 구현체 PropertySourcesPlaceholderConfigurer 가 있는데, 이는 @Value 로 선언한곳에 적절한 프로퍼티 값을 넣어주는 역할
  ...
  - 태스크 실행과 스케줄링
    ...
    - ThreadPoolTaskExecutor 사용방법 확인
      ...
      - ThreadPoolTaskExcecutor 이 현재 기본값임
  ...
  - 캐시 추상화
    ...
    - 동일한 요청이 들어오면 캐시에 보과내두었떤거를 돌려주니, 빠를뿐아니라 리소스를 아낄수있음
  ...
  - @Enable 애노테이션을 이용한 빈 설정정보 모듈화
    ...
    - 빈 설정자
      - 엘리먼트로는 고정된 값만 가능하기때문에, 코드를 이용해 복잡한 설정을 추가해야할 필요가 있을때 사용
    ...
    - ImportSelector
      - 애노테이션 엘리먼트에 따라 import하는 설정정보 클래스를 달리하고 싶다면 ImportSelector를 구현!
  ...
  - 7장을 통한 기타 팁
    ...
    - 인터페이스 타입의 프로퍼티 있으면, 구현체들이 무엇이 있는지 확인해볼것!!
