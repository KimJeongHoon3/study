- 인덱스
  ...
  ---
  - 인덱스 또한 디스크에서 읽어와야한다.. 그래도 인덱스같은경우 읽어오게되면 버퍼풀에 남아있게되고, 버퍼풀과 change버퍼를 통해서 인덱스의 변경이 일어나도 계속해서 최신의 인덱스를 유지한다. 그때에, 인덱스 외의 데이터를 조회하려면 당연히 버퍼풀에는 있지않을것이고, 이를 위해서 디스크로 접근하여 랜덤IO가 일어나게된다! 
