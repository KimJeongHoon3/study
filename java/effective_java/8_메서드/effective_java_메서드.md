...
- 아이템55_옵셔널 반환은 신충히 해라
  - 특정조건에서 값을 반환할 수 없을때 
    - 자바 8 이전
      - null
      - 예외
        - (예외를 생성할때 스택 추적 전체를 캡처하기에 비용이 많이든다함..)
    - 자바 8 이후
      - `Optional<T>` 추가!
        
  - `Optional<T>`
    - T 타입 참조를 하나 담거나, 혹은 아무것도 담지 않을 수 있음
    - 원소를 최대 1개 가질 수 있는 **불변** 컬렉션
      - 의미적으로 1개를 담는 컬렉션이라고 생각하면됨 (Collection 인터페이스를 구현한건 아님)
    - T를 반환해야하지만, 특정 조건에서는 아무것도 반환하지 않아야할 때 T 대신 `Optional<T>` 를 반환하도록 선언하면됨
    - 장점
      - 예외를 던지는 메서드보다 유연하고 사용하기 쉬움
      - null을 반환하는 메서드보다 오류가능성 적음
    - **옵셔널 반환하는 메서드에서 null을 반환하지말아라!!**
      - 옵셔널 도입한 취지를 완전히 무시하는 행위
  - Optaional 반환을 선택하는 기준?
    - 값이 없을때 반드시 어떤 처리를 해야하는 경우 (= 결과가 없을 수 있으며, 클라이언트가 이 상황을 특별하게 처리해야하는 경우)
      - checked Exception을 던지는것과 취지가 비슷함
        - checked Exception을 던지면 클라이언트는 반드시 이에대한 처리를 해주어야하기때문에.. 즉, 반환값이 없을수도 있음을 API 사용자에게 **명확히** 알려주어 클라이언트가 처리하도록 해줌
      - Optional을 통해 클라이언트가 할 수 있는 처리
        - 기본값 설정가능 (빈 Optional을 전달받으면 해당 값으로 가져올수있음. 지연평가로도 가능)
          - orElse
          - orElseGet (지연평가)
        - 비어잇을때 예외 던질 수 있음 (지연평가로도 사용가능)
          - orElseThrow(Supplier)
        - 항상 값이 있다고 가정하여 가져올수도있음 (물론, 이렇게했을때 값이 비어있으면 NoSuchElementException이 발생)
        - filter, map, flatMap, ifPresent 등 사용가능
        - `Stream<Optional<T>>` 로 받을때
          ```java
            streamOfOptionals
            ...
            // 자바9부터는 아래와같이도 사용가능
            streamOfOptionals
                .flatMap(Optional::stream) // Optional.stream은 Optional에 데이터있으면 Stream으로 반환해줌
                
          ```
  - Optional 반환값이 항상 득이냐?
    - 컬렉션, 스트림, 배열, 옵셔널 같은 컨테이너 타입은 옵셔널로 감싸지마라~
      - `Optioanl<List<T>>` 가 아닌, `List<T>`인데, 그냥 빈 List 반환하면됨... 굳이 클라이언트가 Optional 처리코드를 넣을 필요가없다..
    - 성능이 중요한 상황에서는 옵셔널이 맞지않을수도..
      - Optional도 새로 할당하고 초기화해야하는 객체.. 또한 값 꺼내려면 메서드 호출해야하니 한단계 더 거치는샘..
      - 성능에 이슈생기는지 측정해보고 결정하자!
    - 박싱된 기본 타입을 담은 옵셔널을 반환하는 일은 없도록하자!
      - 기본 타입 전용 Optional이 있음
        - OptionalInt, OptionalLong, OptionalDouble
      - 단, 예외로 덜 중요한 기본 타입인 Byte, Boolean, Character, Short, Float은 필요에따라 사용하도록
  - **반환값 외에 Optional을 사용하는 경우는 매우 드물다..**
    - Map의 Key값으로 옵셔널을 사용한다면, 맵안에 키가 없다는 사실을 나타내는 방법이 진짜 키가 없을때와 Optional 빈값일때로 두가지가 되기때문에.. 쓸데없이 복잡성만 높임..
    - 즉, 옵셔널을 컬렉션의 키, 값, 원소 or 배열의 원소로 사용하는게 적절한 상황은 거의 없다!
      - <span style="color:red">`List<Optional<Data>>` 이런 경우는 없으려나..?</span>
  - 옵셔널은 인스턴스 필드에 저장해두는게 필요할때가 있을까?
    - 일반적으로는 필수 필드를 갖는 클래스와, 이를 확장해 선택적 필드를 추가한 하위 클래스를 만들어야함을 암시하는 나쁜냄새
    - 하지만, 필드들이 선택적이고 기본타입이라 값이 없음을 나타내기가 어려울때는 필드를 옵셔널로 사용하는것도 괜찮다
    
---

- 아이템56_공개된 API 요소에는 항상 문서화 주석을 작성하라
  - 필요할때 한번더 보자..
