          ...
          - 참고사항
            - 프록시 모드 (JDK 다이내믹 프록시 vs CGLIB)
              - CGLIB 사용 
                ...
                - ***상속해서 프록시를 만드는 개념***이기때문에, final로 선언된 클래스는 안됨
...
- 4장 스프링 @MVC
  ...
  - 스프링3.1의 @MVC
      ...
      - 요청조건
        ...
        - 종류(총 7가지)
          ...
          - consumes : Content-type 헤더
            - client의 `contentType` 이다. 당연 서버가 응답하는 데이터랑은 하등 상관없음..
            - client가 여기서 json데이터를 바디에셋팅하고 `contentType`을 `plain/text`로 요청보내면 서버에서 json데이터로 인식못한다.. 에러뱉음(`@RequestBody` 에서 셋팅안됨)
            - 이런 실수안하도록하기위해서 `consumes`를 사용해서 조건을 지정해주는게 좋을듯!
          - produces : Accept 헤더
    ...
    - @RequestMapping 핸들러 어뎁터 : RequestMappingHandlerAdpater
      ...
      - RedirectAttributes와 리다이렉트 뷰
        ...
        - ~~당연히 메소드 파라미터로 전달받는것!~~
          - 전달받을때는 `RequestContextUtils.getInputFlashMap` 으로 가져올수있음
    ...
    - @EnableWebMvc와 WebMvcConfigurationSupport를 이용한 @MVC 설정
      - @EnableWebMvc를 사용하면 기본적으로 디폴트 인프라 빈들이 등록되는데, 해당 인프라빈을 변경하거나 수정자를 통해 셋팅이 필요한 즉, 사용자의 커스텀이 필요한 경우는 WebMvcConfigurer(빈 설정자) 나 WebMvcConfigurationSupport를 사용한다
        - WebMvcConfigurer는 인터페이스.. 이를 구현해서 빈으로 등록되어야 빈 설정자로 사용가능 
          ...
          - WebMvcConfigurer는 여러개 등록되어도 된다. 여러개 등록되면 빈 설정자(Configurer)를 통해 등록한 설정정보들이 모두 사용된다! (그렇기에, 용도에 맞는 설정자를 셋팅해놓고 필요에 따라서 설정클래스에 등록해서 사용하는게 좋겠지 ex. SecurityConfigurer, CustomerHnadlerConfigurer..)
  ...
  - 4장을 통한 기타 팁
    ...
    - 하나의 사진으로 깔끔하게 정리
      ...
      - 필터 VS 인터셉터
        - 필터
          - 필터는 스프링컨테이너에서 관리되는게아닌, 서블릿컨테이너에서 관리 (J2EE 표준 스펙)
          - 그렇기에 필터는 스프링과 독립적인(그렇다고 빈 주입이 안되는것은 아님), 전역적인 설정 셋팅에 좋음.
            - 공통된 보안, 인증/인가 관련..
            - 모든 요청에 대한 로깅
          - HttpServletRequest나 HttpServletResponse에 대한 조작가능 (`chain.doFilter`에서 객체를 직접 넘겨줄수있기때문에 Wrapper와 같은것을 사용가능)
        - 인터셉터
          - 인터셉터는 스프링컨테이너에서 관리
          - Controller 단위의 세부적인 인증 인가 가능
          - 특정 API의 호출에 대한 로깅가능
          - 컨트롤러로 넘겨주는 데이터 조작가능
          - Controller(핸들러 메서드) 호출전에 타게됨
        - [정리매우굿](https://mangkyu.tistory.com/173)
...
----
  ...
  - 서블릿 컨테이너와 스프링 컨테이너 (서블릿컨텍스트, 웹어플리케이션컨텍스트)
    - 서블릿 컨테이너는 톰켓이나 제티와 같은 was가 만들어주게되고 이 was는(web.xml을 읽어서..) ServletContext를 생성 및 소유하게된다.(DispatcherServlet에서 가지는 어플리케이션 컨텍스트가 아니다) 이 ServletContext에는 스프링 루트 어플리케이션 컨텍스트 (applicationContext.xml)를 등록하며 또한 서블릿을 등록할 수 있다. 여기서 스프링에서는 DispatcherServlet(서블릿구현체)을 사용하여 서블릿에 등록하게되는데, DispatcherServlet에서는 servlet-context.xml을 통해 생성된 어플리케이션컨텍스트를 가지고 있다. 해당 어플리케이션컨텍스트는 ServletContext에 등록한 루트어플리케이션 컨텍스트를 부모로 가지고있어서, 루트어플리케이션 컨텍스트의 빈들을 가져와서 사용할 수 있게된다. (여기 루트어플리케이션 컨텍스트엔 보통 서비스단 등의 빈들이 등록된다. Servlet쪽과는 독립적으로..)
      - ContextLoaderListener를 통해서 루트어플리케이션컨텍스트를 서블릿컨텍스트에 등록한다
      ...
      - 스프링부트는 좀 다르다. 서블릿컨테이너가 후킹하는 구조로 되어있는 것과 달리 내장 서블릿컨테이너를 사용하는데, 이렇게 되었을때 필터와 같은 부분을 스프링 설정에서 인지할수있다
        - 공식문서 : Spring Boot follows a different initialization sequence. Rather than hooking into the lifecycle of the Servlet container, Spring Boot uses Spring configuration to bootstrap itself and the embedded Servlet container. Filter and Servlet declarations are detected in Spring configuration and registered with the Servlet container. 
    - ????
      - 루트어플리케이션컨텍스트와 서블릿어플리케이션컨텍스트가 언제 부모관계로 셋팅되는지..
        - `AbstractAnnotationConfigDispatcherServletInitializer` web.xml 대신 자바config로 만드는방법인데, 여기서만 봤을때 어떻게 둘의 관계를 부모로 만들어주는지 모르겠다..
        - => DispatcherServlet이 초기화될때 셋팅됨
        ```java
          // FreameworkServlet 클래스에 있음 ( DispatcherServlet extends FreameworkServlet)
          protected WebApplicationContext initWebApplicationContext() {
            ...
            /**
            만약 서블릿어플리케이션컨텍스트를 등록하지않았다면, 루트어플리케이션컨텍스트를 같이 사용하게된다..
            아래 if문에 걸려서 그거 사용하는듯
    ...
    - WAS가 실행되면, servletContext를 기반으로 서블릿 컨테이너를 올리게된다.. 
      ...
      ```java
        public interface WebApplicationInitializer{ //이거를 WAS가 호출하는듯. 좀더 엄밀하게는 spring이 구현한 ServletContainerInitializer의 구현체(SpringServletContainerInitializer)가 호출해줌
  ...
  - ServletContext?
    ...
    - WAS에 등록된 웹 애플리케이션 단위로 컨텍스트가 생성.. 즉, WAS에 3개의 웹어플리케이션이있다면, DispatcherServlet이 3개 (그에 따라 DispatcherServlet이 가지고있는 applicationContext도 3개)
...
- spring xml 설정
  - `mvc:annotation-driven` ( 출처 : https://velog.io/@hanblueblue/Spring-mvcannotation-driven )
    - `message-converters`
      - http 요청메세지 본문과 http 응답메세지 본문을 어떻게 컨버팅할것인가
    - `argument-reslovers`
      - 컨트롤러에 들어가기 전에 클라이언트로부터 받은 파라미터들을 원하는 것으로 가공해서 컨트롤러로 전달
      - `HandlerMethodArgumentResolver`를 구현한 클래스를 사용해야함
    - `async-support`
      - 비동기 사용을 위한 설정셋팅
      - 자세한내용은 출처참고
    - `path-matching`
    - `return-value-handlers`
- 톰캣의 jsessionid 만드는 방법
  - https://semtax.tistory.com/92


---

- 인터셉터가 특정 Handler에 등록되는과정 (spring 4.3.14 기준)
  - 요청이 들어오면 DispatcherServlet에서 HandlerMapping 구현체들중에 적절한 HandlerMapping 구현체를 찾고
  - HandlerMapping 구현체에서 적절한 Handler를 만들고(정확히는 HandlerExecutionChain),
    - 여기서 만들때, HandlerMapping의 추상클래스마다 가지고 있는 필수기본 인터셉터들 적절하게 넣어주고 (ex. `AbstractUrlHandlerMapping`)
    - 사용자가 정의한 인터셉터들을 알맞게 넣어줌 (`AbstractHandlerMapping`의 adaptedInterceptors)
      - 즉, 기본 HandlerMapping의 전략구현체들인 RequestMappingHandlerMapping과 SimpleUrlHandlerMapping에서 가지고있는 인터셉터들이 다름
  - HandlerExecutionChain의 handler를 기반으로 이 핸들러를 실행시키는 적절한 HandlerAdpator를 찾아서 실행 (여기서 인터셉터들이 실행되고하겠지) 
  - **스프링 5.x 에서는 적용 방법이 달라짐!!**
    - 위의 적용방법은 동일하나, HandlerMapping 구현체를 빈으로 등록하는 과정에서 인터셉터를 넣어주는 로직이 변경됨. 
    - 인터셉터를 등록하면, 모든 HandlerMapping 구현체들이 해당 인터셉터를 가지게됨. (즉, 기본적으로(default) 모든 handlerMapping구현체는 동일한 인터셉터를 가진다고보면됨)
    - 특정 HandlerMapping에서만 인터셉터를 적용시키고자한다면, HandlerMapping을 오버라이딩 해야한다.. (하지만 굳이 그럴필요없이 그냥 excludePath를 잘 설정하자)
    - 참고로 스프링4.x에서는 사용자가 등록한 인터셉터는 `RequestMappingHandlerMapping` 에서만 적용되었음
      - 챗GPT 답변
        - ![](2023-06-16-11-39-53.png)
    - 소스 확인
      - WebMvcConfigurationSupport.resourceHandlerMapping 에 인터셉터 적용하는 소스가 변경
        - 5.3.26
          ```java
              @Bean
              @Nullable
              public HandlerMapping resourceHandlerMapping(@Qualifier("mvcContentNegotiationManager") ContentNegotiationManager contentNegotiationManager, @Qualifier("mvcConversionService") FormattingConversionService conversionService, @Qualifier("mvcResourceUrlProvider") ResourceUrlProvider resourceUrlProvider) {
                  ...
                  if (handlerMapping == null) {
                      ...
                      handlerMapping.setInterceptors(this.getInterceptors(conversionService, resourceUrlProvider)); // 공통으로 가져옴.. 여기 handlerMapping뿐 아니라 다른 handler도 동일한 메서드 사용 (RequestMappingHandlerMapping 등록할때도 마찬가지..)
        ...
        - 4.3.12
          ```java
              ...
              public HandlerMapping resourceHandlerMapping() {
                  ResourceHandlerRegistry registry = new ResourceHandlerRegistry(this.applicationContext, this.servletContext, this.mvcContentNegotiationManager());
                  this.addResourceHandlers(registry);
                  AbstractHandlerMapping handlerMapping = registry.getHandlerMapping();
                  if (handlerMapping != null) {
                      ((AbstractHandlerMapping)handlerMapping).setPathMatcher(this.mvcPathMatcher());
                      ((AbstractHandlerMapping)handlerMapping).setUrlPathHelper(this.mvcUrlPathHelper());
                      ((AbstractHandlerMapping)handlerMapping).setInterceptors(new Object[]{new ResourceUrlProviderExposingInterceptor(this.mvcResourceUrlProvider())}); // 인터셉터를 직접 등록.. 사용자가 등록한걸 전달해주지않음. 반면, RequestMappingHandlerMapping 에서는 사용자가 등록한것들 interceptor에 넣어주는 로직이 있음..
                      ((AbstractHandlerMapping)handlerMapping).setCorsConfigurations(this.getCorsConfigurations());
                  } else {
              ...
              public RequestMappingHandlerMapping requestMappingHandlerMapping() {
                  ...
                  mapping.setOrder(0);
                  mapping.setInterceptors(this.getInterceptors()); // 요기
